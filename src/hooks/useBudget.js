...
import { computeWalletBalance, computeWalletBalances, isVisibleManagedExpense, moneyNumber } from '../utils/budgetMath'
...
export function useBudget(options = {}) {
  ...
  useEffect(() => {
    ...
    const setupSubscriptions = async () => {
      ...
      const updateSyncState = (key, metadata) => {
        ...
        const hasPendingWrites = values.some((entry) => entry?.hasPendingWrites)
        const isFromCache = values.length > 0 && values.every((entry) => entry?.fromCache)
        setSyncState((current) => ({
          hasPendingWrites,
          isFromCache,
          lastSyncedAt: !hasPendingWrites && !isFromCache ? Date.now() : current.lastSyncedAt
        }))
      }
      ...
      try {
        ...
        const unsubscribeNetWorthSnapshots = netWorthSnapshotService.subscribeToNetWorthSnapshots(
          user.uid,
          (items) => { if (isMounted) setNetWorthSnapshots(items || []) },
          (metadata) => updateSyncState('netWorthSnapshots', metadata)
        )
  ...
  const addIncome = useCallback(async (income) => {
    ...
    try {
      setError(null)
      return await incomeService.addIncome(income, user.uid)
    } catch (err) {
      setError(err.message)
      throw err
    }
  ...
  const addRecurringIncome = useCallback(async (recurringIncome) => {
    ...
    try {
      setError(null)
      return await recurringIncomeService.addRecurringIncome({
        ...recurringIncome,
        lastPostedDate: null,
  ...
  }, [enableRecurringIncomes, normalizeDateKey, user])
  ...
  const addExpense = useCallback(async (expense) => {
    ...
    try {
      setError(null)
      return await expenseService.addExpense(compactExpensePayload(expense), user.uid)
    } catch (err) {
      setError(err.message)
      throw err
    }
  ...
  const addSavings = useCallback(async (savingsGoal) => {
    ...
    try {
      setError(null)
      return await savingsService.addSavings(savingsGoal, user.uid)
    } catch (err) {
      setError(err.message)
      throw err
    }
  ...
  const addToSavingsGoal = useCallback(async (savingsId, amount, options = {}) => {
    if (!user) throw new Error('Please sign in to add savings funds')

    const goal = savings.find((entry) => entry.id === savingsId)
    const numericAmount = moneyNumber(amount)
    ...
    if (!sourceWalletId) {
      await savingsService.updateSavings(savingsId, {
        currentAmount: moneyNumber(goal.currentAmount) + numericAmount
      })
      return
    }
    ...
    const computedSourceBalance = computeWalletBalance(sourceWallet, { incomes, expenses, transfers })
    ...
    try {
      setError(null)
      await savingsService.fundSavingsFromWallet({
        savingsId,
  ...
  const addCategory = useCallback(async (category) => {
    ...
    try {
      ...
      return await categoryService.addCategory(category, user.uid)
    } catch (err) {
      setError(err.message)
      throw err
    }
  ...
  const addWallet = useCallback(async (wallet) => {
    ...
    const tempWallet = {
      ...
      creditApr: wallet.creditApr || null,
      creditMinimumPayment: wallet.creditMinimumPayment || null,
  ...
  const addSubscription = useCallback(async (subscription) => {
    ...
    try {
      ...
      return await subscriptionService.addSubscription({
        ...subscription,
  ...
  const addTransfer = useCallback(async (transfer) => {
    ...
    try {
      setError(null)
      return await transferService.addTransfer(transfer, user.uid)
    } catch (err) {
      setError(err.message)
      throw err
    }
  ...
  const addInvestment = useCallback(async (investment) => {
    ...
    try {
      setError(null)
      return await investmentService.addInvestment(investment, user.uid)
    } catch (err) {
      setError(err.message)
      throw err
    }
  ...
  const totalInvestments = useMemo(() => (
    investments.reduce((sum, investment) => (
      sum + ((parseFloat(investment.quantity || 0) || 0) * Number(investment.currentValue ?? investment.purchasePrice ?? 0))
    ), 0)
  ), [investments])
  ...
  const walletBalances = useMemo(() => (
    computeWalletBalances(wallets, { incomes, expenses: visibleExpenses, transfers })
  ...
  const categoryBudgets = useMemo(() => (
    categories.map((category) => {
      ...
      const carryover = parseFloat(matchingBudget?.carryover || 0)
      const effectiveAmount = amount + carryover
      const hasBudget = effectiveAmount > 0
      const utilization = hasBudget ? (spent / effectiveAmount) * 100 : 0
      const now = new Date()
      const daysRemaining = selectedYear === now.getFullYear() && selectedMonth === now.getMonth()
        ? new Date(selectedYear, selectedMonth + 1, 0).getDate() - now.getDate() + 1
        : null
      return {
        ...
        hasBudget,
        utilization,
        remaining: hasBudget ? effectiveAmount - spent : null,
        dailyAllowance: hasBudget && daysRemaining ? Math.max(effectiveAmount - spent, 0) / daysRemaining : null,
        status: !hasBudget ? 'unset' : utilization >= 100 ? 'over' : utilization >= 80 ? 'warning' : 'healthy'
      }
    })
  ), [budgets, categories, filteredExpenses, selectedMonth, selectedYear])
  ...
  const netWorthSummary = useMemo(() => {
    ...
    const investmentTotals = summarizeByCurrency(
      investments,
      (investment) => (parseFloat(investment.quantity || 0) || 0) * Number(investment.currentValue ?? investment.purchasePrice ?? 0),
  ...
  const totalNetWorth = useMemo(() => (
    netWorthSummary.reduce((sum, entry) => sum + Number(entry.total || 0), 0)
  ), [netWorthSummary])

  useEffect(() => {
    if (!user || netWorthSummary.length !== 1) return

    netWorthSnapshotService.upsertSnapshot({
      monthKey: selectedMonthKey,
      currency: netWorthSummary[0].currency,
      total: netWorthSummary[0].total
    }, user.uid).catch((err) => {
      console.error('Failed to save net worth snapshot:', err)
    })
  }, [netWorthSummary, selectedMonthKey, user])
  ...
  const creditCardSummaries = useMemo(() => (
    walletBalances
      ...
      .map((wallet) => {
        ...
        const apr = parseFloat(wallet.creditApr || 0)
        const plannedPayment = parseFloat(wallet.creditMinimumPayment || 0)
        ...
        if (spent > 0 && plannedPayment > 0) {
          ...
          while (balance > 0.01 && payoffMonths < 600) {
            ...
            balance = balance + interest - plannedPayment
            payoffMonths++
            if (plannedPayment <= interest) {
              payoffMonths = null
              payoffInterest = 0
              break
            }
