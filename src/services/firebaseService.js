...
export const expenseService = {
  ...
  addExpense: async (expense, userId) => {
    ...
    const isManagedSubscriptionExpense = (
    ...
    if (
      isSubscriptionExpense &&
      expense?.postingSource &&
      !isManagedSubscriptionExpense
    ) {
      throw new Error('Automatic subscription expense creation is disabled')
    }

    if (isManagedSubscriptionExpense && expense?.date) {
      const docId = `subscription_${userId}_${expense.subscriptionId}_${String(expense.date).replace(/[^a-zA-Z0-9_-]/g, '_')}`
      const docRef = doc(db, COLLECTION_NAMES.EXPENSES, docId)
      await setDoc(docRef, {
        ...expense,
        userId,
        createdAt: new Date()
